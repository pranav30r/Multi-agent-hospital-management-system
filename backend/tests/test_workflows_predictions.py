...
@pytest.mark.asyncio
async def test_workflow_definition_and_lifecycle(client):
    """Test starting a workflow instance and advancing its steps."""
    # List definitions
    res = await client.get("/api/v1/workflows/definitions")
    ...
    start_res = await client.post("/api/v1/workflows/instances", json=start_payload)
    ...
    adv_res = await client.post(
        ...
        json={"performed_by": "DOC-001", "notes": "Registration verified"}
    )
...
@pytest.mark.asyncio
async def test_predictions_record_and_metrics(client):
    """Test recording a model inference run and querying metrics."""
    # Record prediction
    pred_payload = {
    ...
    rec_res = await client.post("/api/v1/predictions/record", json=pred_payload)
    ...
    # Query latest
    latest_res = await client.get("/api/v1/predictions/latest?prediction_type=SURGE_FORECAST")
    assert latest_res.status_code == 200
    assert len(latest_res.json()) >= 1

    # Query metrics
    metrics_res = await client.get("/api/v1/predictions/metrics")
