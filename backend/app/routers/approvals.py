...
from app.models import ApprovalItem, AgentDecision, AuditLog
...
class ApprovalActionRequest(BaseModel):
    ...
    reviewed_by: str = Field(..., example="DOC-003")
...
async def review_approval_item(approval_id: str, req: ApprovalActionRequest, db: AsyncSession = Depends(get_db)):
    """
    Submit human decision (APPROVE / MODIFY / REJECT) for a pending AI recommendation.
    """
    res = await db.execute(select(ApprovalItem).where(ApprovalItem.id == approval_id))
    ...
    audit = AuditLog(
        ...
        changed_by=req.reviewed_by,
        change_reason=f"Human Review: {req.action}" + (f" (Modified: {req.modification})" if req.modification else f" (Reason: {req.rejection_reason})" if req.rejection_reason else ""),
    ...
    logger.info(f"APPROVAL RESOLVED: Item {approval_id} -> {req.action} by {req.reviewed_by}")
    return item
